...
- **外观主题：全局换肤** —— 「设置 → 外观主题」一键开启，用内置的一套自定义外观给整个 Obsidian 换肤、关闭切回原主题。冷色沉静风（呼应插件的 NORD 配色），保留描边卡片质感，并关掉了重度毛玻璃换纯色卡片——**更清爽、滚动顺滑**。内置 **5 套大牌配色**（Catppuccin / Tokyo Night / Rosé Pine / Everforest / Nord，全部官方色值、暗色 + 亮色都有），点色块即时整套换肤，还能直接切 ☀️ 亮 / 🌙 暗。走 Obsidian 原生主题机制，不和你当前主题冲突。基于开源主题 Border（© 2022 Akifyss · MIT）定制。
