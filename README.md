...
<td width="50%" valign="top">
  ...
  <b>📈 项目日志</b><br>
  <sub>项目→主题→每日条目三级管理 + 项目级待办清单；四视图（折叠日志 / 按天 / 热力图 / 甘特泳道），主题区间自动推导，重叠即并行。</sub>
</td>
<td width="50%" valign="top">
...
| 🌐 联网要求 | **激活时联网校验一次**（只传 Key + 匿名设备标识，**不传任何笔记内容**）；激活成功后**离线运行**，断网也有 3 天宽限 |
...
- ✅ **不收集使用统计、不监控行为**
- ✅ **激活时联网一次**：仅向激活服务器上传 License Key + 一个**匿名设备标识**（随机串，存本地、不随 vault 同步、不含任何个人信息），用于校验授权和设备数上限（最多 3 台）；激活成功后离线运行，**不再连接服务器**（断网另有 3 天宽限自动重试）
