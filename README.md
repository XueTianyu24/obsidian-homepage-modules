...
<div align="center">
  ...
  <b>🗂️ 词组</b> · <sub>以「组」为单位组织背单词——把共享同一 <b>词缀 / 词根 / 形近</b> 的词圈成一个命名的组，以组记忆；<b>一个词可同时属于多个组</b>（词↔组多对多），成员词显示「也在这些组」。组按类型分色（词缀蓝 / 词根绿 / 形近橙）+ 建组 / 类型筛选 / 搜索。<b>加词一个入口</b>：库里有就搜出来点选加入，<b>库里没有直接新建并自动查词</b>（音标 / 中文释义 / 例句自动补全，复用背单词的 ECDICT 离线词库）——不用先在背单词加过、也不用手抄释义。与「背单词」<b>共享同一份词库</b>，组只存词的引用，词不重录。</sub>
</div>
