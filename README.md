...
- **高亮标注：选中浮出工具条** —— 编辑模式选中文字，立刻浮出迷你工具条：**5 色文字高亮**（圆点）+ **5 色整段高亮**（方块，把整段包成彩色 callout 色块）+ 🧹 一键清除。颜色直接写进笔记（行内样式 / Obsidian 原生 callout），**以后就算卸载插件，高亮也不掉色**。对已高亮的文字点新颜色即换色；每个颜色都有命令可绑快捷键。仅桌面端。
