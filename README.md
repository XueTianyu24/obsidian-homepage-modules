...
不想为每个工具单独开页面？桌面端点**右下角状态栏「🖥 工作台」**（就在「🏠 主页」旁边）/ 命令「打开工作台」一键进入（移动端走左侧 ribbon）：左侧栏最上是 **🏠 主页**（打开工作台默认就显示它——一般只需开工作台、不用再单独开主页），下面分组列出全部重型工具（每日 / 学习 / 自我 / 其它），点击在主区一次显示**一个完整工具**、工具间一键跳转——像 VS Code / Notion 那样的「侧栏 + 单一主内容区」。**工具页还能叠加多个模块一起看**：页面底部「＋ 添加模块」把另一个模块（任意模块或小部件）叠到当前工具下方（如「日历日程」叠上「学术 DDL」倒计时，一屏看全安排 + 截止），叠加的模块可 `↑↓` 调序 / `×` 移除，配置记住、重开还在。侧栏可一键折叠成图标栏腾出空间，自动记住你上次打开的工具；**侧栏工具随你增删、定制自己的工作台**——「工具」旁 ✏️ 进编辑模式：不常用的工具 **✕ 移出**、底部 **＋ 工具** 把移出的工具或小部件（问候 / 倒计时 / 最近文件…）加进侧栏，还能**拖拽调整顺序、自定义分组**（改组名 / 分组排序 / 新建删除）、一键「↺ 恢复默认」，行距紧凑到工具一屏放完、不出滚动条。工作台里 **`Alt + ↑/↓` 直接切工具**（像 IDE 切标签）；桌面端当前工具标题条右侧可「↗ 新标签打开」把它单独拉到新标签、与笔记并排看。**移动端**顶部则收成一行紧凑条（`☰ 当前工具 · ⚙️ · ↻`，点 ☰ 展开工具列表选择、选完自动收起），并去掉单独标题栏，把屏幕尽量让给工具内容。与现有「主页」仪表盘**并存**，按习惯任选。
