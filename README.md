...
> 当前版本 **v0.13.0**：**15 个模块** + 2 个插件级功能（Ctrl + 滚轮缩放 / 番茄状态栏）。
> **主页升级为独立 ItemView**（左侧 🏠 ribbon / 命令「打开主页」），铺满面板不再受
> 可读行宽限制；一键应用推荐布局（10 个常用模块），开机即用。
...
| 模块 id | 名称 | 一句话 |
|---|---|---|
| `home` | 主页 | 用 grid 布局把多个模块组合到一张主页。**两种打开方式：独立标签页（推荐，左侧 🏠 ribbon / 命令「打开主页」，铺满面板）+ 代码块版（嵌入笔记，受可读行宽限制）**。首次打开可一键应用推荐布局；编辑模式 ↑↓ 调位置 / ▢▭ 切单列整行 / + 添加模块 / 列数 1/2/3 |
