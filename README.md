...
- **PDF 导出增强** —— 导出 PDF 时让**表格不跨页**（整张尽量落一页，放不下的大表保证「行不腰斩 + 表头每页重复」），可选 callout / 代码块 / 图片也不跨页；命令「插入 PDF 目录页」给当前笔记**自动生成一页可点击跳转的目录**（`[[#标题]]` 链接 + 层级缩进），「移除 PDF 目录页」一键撤掉。设置里配目录标题 / 收录层级 / 是否单独成页。
