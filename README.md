...
番茄、自律、健身、精听听写、词根词缀、词组、天气、会议 DDL、每日安排、日历、统计、倒计时等 **26 个交互模块**；再用「主页」把它们拼成一张
...
不想为每个工具单独开页面？桌面端点**右下角状态栏「🖥 工作台」**（就在「🏠 主页」旁边）/ 命令「打开工作台」一键进入（移动端走左侧 ribbon）：左侧栏最上是 **🏠 主页**（打开工作台默认就显示它——一般只需开工作台、不用再单独开主页），下面分组列出全部 17 个重型工具（每日 / 学习 / 自我 / 其它），点击在主区一次显示**一个完整工具**、工具间一键跳转——像 VS Code / Notion 那样的「侧栏 + 单一主内容区」。**工具页还能叠加多个模块一起看**：页面底部「＋ 添加模块」把另一个模块（任意模块或小部件）叠到当前工具下方（如「日历日程」叠上「学术 DDL」倒计时，一屏看全安排 + 截止），叠加的模块可 `↑↓` 调序 / `×` 移除，配置记住、重开还在。侧栏可一键折叠成图标栏腾出空间，自动记住你上次打开的工具；**布局随你定**——「工具」旁 ✏️ 进编辑模式，**拖拽调整工具顺序、自定义分组**（改组名 / 分组排序 / 新建删除），行距紧凑到全部工具一屏放完、不出滚动条。工作台里 **`Alt + ↑/↓` 直接切工具**（像 IDE 切标签）；桌面端当前工具标题条右侧可「↗ 新标签打开」把它单独拉到新标签、与笔记并排看。**移动端**顶部则收成一行紧凑条（`☰ 当前工具 · ⚙️ · ↻`，点 ☰ 展开工具列表选择、选完自动收起），并去掉单独标题栏，把屏幕尽量让给工具内容。与现有「主页」仪表盘**并存**，按习惯任选。
...
装好后，命令面板（Ctrl / Cmd + P）搜「**创建效率工具页面**」，或在 **设置 → Homepage Modules → 快速开始** 点「创建页面」：插件会在你 vault 的「效率工具」目录下，为日记 / 番茄 / 待办 / 背单词 / 词组 / 自律 / 精神内核 / 天气 / 健身 / 健身日记 / 精听听写 / 词根词缀 / 项目日志 / 提示词库 / 个人信条 / 学术 DDL / 每日安排 / 日历各建一个**开箱即用**的页面 + 一张总览导航页。
