...
<td width="50%" valign="top">
  ...
  <b>🍅 番茄专注</b><br>
  <sub>时间戳法计时（关掉 Obsidian 也不丢进度）+ <b>沉浸模式（正计时）</b>：点「∞ 不限时」从 0 往上走、不自动结束，按你设的间隔（每 30m / 60m / 90m…）轻声提醒一次、不打断，干完点「✓ 结束」记一条（桌面悬浮窗缩成迷你小条时，沉浸中也常显这个「✓」，不用还原窗口）。另有桌面悬浮窗 + 全年热力图 + 历史记录可单条删除。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
