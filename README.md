...
| `discipline` | 自律打卡：对自己的行为约束 —— 要做 / 限额 / 戒断三种类型，按类型分组 + 组内 ↑↓ 排序，周视图 + 顶部汇总（达标 / 越线 / 最长在戒）+ 每项 7 天迷你热力；每项 📊 详情含 53×7 全年热力图 + 四卡累计统计；要做类可自定义指标（跑步距离 / 配速 / 用时，含业内 m'ss" 格式），打卡时弹表单填值、详情按聚合（总计/最大/最小/平均）显示 |
| `psyche` | 精神内核：用 SVG 自动布局的树组织你的价值观 / 原则 / 应对模式（替代手画 Canvas）。节点 = label + emoji + 颜色 + 详细说明；点节点 → 详情弹窗 → ✎ 编辑 / ➕ 加子节点 / ✕ 删除；▼/▶ 折叠子树；可关联到自律打卡项形成"内核↔行动"闭环 |
